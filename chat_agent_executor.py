...
from langchain_core.messages import AIMessage, BaseMessage, SystemMessage, ToolMessage ,RemoveMessage
...
def create_react_agent(
    ...
    *,
    ...
    def delete_messages(state: AgentState, config: RunnableConfig) -> AgentState:
        messages = state["messages"]
        if len(messages) > 2:
            messages_del = [RemoveMessage(id=m.id) for m in messages[:-2]]
        else:
            messages_del = []
        print("messages_del", messages_del)
        
        return {"messages":  messages_del}
    ...
    def should_delete(state: AgentState) -> Union[str, list,dict]:
        messages = state["messages"]
        last_message = messages[-1]
        # If there is no function call, then we finish
        if hasattr(last_message, "name") and last_message.name == 'format_data':
    ...
    workflow.add_node("delete_messages",RunnableCallable(delete_messages))
